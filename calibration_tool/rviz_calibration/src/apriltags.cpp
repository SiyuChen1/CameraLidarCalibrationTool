    ...
    publish_tf_(getAprilTagOption<bool>(pnh, "publish_tf", false))
{
  ...
  for(int i = 0;i < 587; i++){
    standalone_tag_descriptions[i]["id"] = i;
    standalone_tag_descriptions[i]["size"] = 0.15;
    standalone_tag_descriptions[i]["name"] = "tag36h11_" + std::to_string(i);
  }
  ...
  // XmlRpc::XmlRpcValue tag_bundle_descriptions[2][3] = {0}; 
  ...
  pnh.setParam("camera_frame", "camera");
...
QString TagDetector::Mat2QString(cv::Mat &mat)
{
    QString s = "";
...
rviz_calibration::AprilTagDetectionArray TagDetector::detectTags (
  ...
  double fx = 960; // focal length in camera x-direction [px]
  double fy = 540; // focal length in camera y-direction [px]
  double cx = 700; // optical center x-coordinate [px]
  double cy = 700; // optical center y-coordinate [px]
  ...
  // detected tag
  rviz_calibration::AprilTagDetectionArray tag_detection_array;
  ...
  for (int i=0; i < zarray_size(detections_); i++)
  {
    ...
    rviz_calibration::AprilTagDetection tag_detection;
    rviz_calibration::AprilTagPoint tag_point;
    tag_point.center[0] = detection->c[0];
    tag_point.center[1] = detection->c[1];
    tag_point.corner[0] = detection->p[0][0];
    tag_point.corner[1] = detection->p[0][1];
    tag_point.corner[2] = detection->p[1][0];
    tag_point.corner[3] = detection->p[1][1];
    tag_point.corner[4] = detection->p[2][0];
    tag_point.corner[5] = detection->p[2][1];
    tag_point.corner[6] = detection->p[3][0];
    tag_point.corner[7] = detection->p[3][1];
    tag_detection.id.push_back(detection->id);
    tag_detection.size.push_back(tag_size);
    tag_detection.point.push_back(tag_point);
